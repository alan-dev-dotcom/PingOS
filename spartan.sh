...
# Driver list (matches the DRIVER_OBJS in the original Makefile)
DRIVER_SOURCES=(
    "drivers/vga.c"
    "drivers/keyboard.c"
    "drivers/mouse.c"
...
compile_kernel() {
    ...
    # Split CFLAGS for execution safety
    ...
    for src in "${DRIVER_SOURCES[@]}"; do
        if [ -f "$src" ]; then
            # Extract object file path (e.g. drivers/vga.o)
            local obj="${src%.c}.o"
            ...
            echo "Warning: Driver file $src not found, skipping."
        fi
    done
